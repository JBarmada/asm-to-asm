...
print(f"  - humaneval_dir: {humaneval_dir}")
print(f"  - ll_base_dir: {ll_base_dir}")
print(f"  - exe_base_dir: {exe_base_dir}")
...
def main():
    ...
    qemu_path = shutil.which("qemu-riscv64")
    ...
    arch_suffix = "rv64"  # for RISC-V 64-bit
    output_dir = os.path.join(exe_base_dir, opt_str, f"{output_toolchain_name}_{arch_suffix}")
    json_results_path = os.path.join(results_dir, f"{opt_str}_{output_toolchain_name}_{arch_suffix}_results.json")
    ...
    for i in problems_to_run:
        ...
        try:
            ...
            print(f"  [1/2] Cross-compiling lifted IR for {problem_name} RISCV...")
            # compile_cmd = [
            ...
            compile_cmd = [
                ...
                "-static",  # Fully static to avoid sysroot issues
                opt_level_flag, "-o", output_exe, ll_file, test_c_file, "-lm"
            ]
            ...
            qemu_cmd = ["qemu-riscv64", output_exe]
